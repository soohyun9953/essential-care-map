...
const MOCK_EMERGENCY_DATA: Record<string, EmergencyHospitalStatus[]> = {
  영월군: [
    {
      기관명: '영월의료원 (지역응급의료기관)',
      기관코드: 'GW001',
...
export async function fetchRealtimeEmergencyStatus(params: {
  ...
  if (apiKey && apiKey.trim() !== '') {
    try {
      ...
      if (params.sido) url.searchParams.append('STAGE1', params.sido);
      if (params.sgg) url.searchParams.append('STAGE2', params.sgg);
