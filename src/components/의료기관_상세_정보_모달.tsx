...
export const 의료기관_상세_정보_모달: React.FC<의료기관_상세_정보_모달_속성> = ({
  ...
  // 근거가 '청구실적'(심평원 공개 목록)이면 추정이 아니므로 표기를 구분
  const render_status_badge = (status: 서비스_운영_상태, 근거?: '청구실적' | '추정') => {
    const 실적 = 근거 === '청구실적';
    if (status === '운영') {
      return (
          ...
          {active_tab === 'services' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs text-slate-500 pb-1">
                <span>9대 핵심 의료서비스 운영 여부 (분만: 심평원 분만 청구 실적 · 그 외: 기관 유형·규모 기반 추정 · 방문 전 기관 확인 필요)</span>
