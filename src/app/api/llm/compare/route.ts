...
- 권한이 필요한 경우 Google AI Studio(https://aistudio.google.com/app/apikey)에서 신규 키를 발급받아 재등록하시면 즉시 정상 작동합니다.`,
    ...
    const fetch_local_sllm = async () => {
      ...
      // 수치는 앱 내장 지침 코퍼스와 일치하는 것만 기재하며, 판정 문구는 실제 지표값으로 산출

      const 응급_판정 = (region_stats.emergency_rate ?? 0) >= 30 ? '법정 기준선 30% 이상 (응급취약 기준 해당)' : '법정 기준선 30% 미만';
      const RI_판정 = (region_stats.ri_rate ?? 100) < 30 ? '기준선 30% 미만 (취약)' : '기준선 30% 이상';
      const 분만율_판정 =
        (region_stats.maternity_rate ?? 100) < 40
          ? `관내 분만율이 ${region_stats.maternity_rate}%로 선정기준(40% 미만)에 해당하므로 지원사업 신청 요건을 검토할 수 있습니다.`
          : `관내 분만율이 ${region_stats.maternity_rate}%로 분만율 기준(40% 미만)에는 해당하지 않으며, 60분 미도달 인구비율 기준 해당 여부를 함께 확인해야 합니다.`;
...
2. 신청 자격 및 필수 요건
  ...
  - 우선 배정: 응급 60분 미도달율 30% 이상 또는 관내 의료이용률(RI) 30% 미만 지역 (${region_name}: 응급 미도달 ${region_stats.emergency_rate}% — ${응급_판정})
  - 인건비: 1인당 국비 50%(최대 2억 5천만원 한도) + 지방비 50% 1:1 매칭 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - 당직수당: 심야 당직수당 월 최대 500만원 추가 보조 (내장 지침 코퍼스 기준, 원문 확인 필요)

3. 현장 행정 조치 권고
  - 사업 공고 전 관내 결원 필수진료과 수요 조사를 완료하고 지자체 예산 매칭 계획을 사전 확보하시기 바랍니다.`;
...
2. 정부 공공보건의료 평가 가점 기준
  - 취약 기준: RI 30% 미만 기초자치단체는 2순위 응급취약지로 판정 (${region_name}: 현재 ${region_stats.ri_rate}% — ${RI_판정})
  - 성과 평가: 기준 취약선(30%) 미달 병원이 전년 대비 1%p 이상 상승 시 해당 지표 만점(10점) (내장 지침 코퍼스 기준, 원문 확인 필요)`;
        } else if (query.includes('분만취약지') || query.includes('A등급') || query.includes('B등급')) {
          detailed_answer = `[사전 작성 정책 질의응답 요약 · AI 모델 미사용]
질문: "${query}"

1. 분만취약지 선정 기준 및 등급별 지원
  - 선정 기준: 관내 분만율 40% 미만 또는 분만 가능 산부인과 60분 미도달 인구 30% 이상 시군구
  - A등급: 분만산부인과 개설 및 매년 5억원 운영비 연속 지원 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - B등급: 자체 분만 역량이 없는 지역에 외래산부인과 설치 지원, 고위험 산모 이송체계 및 모자의료센터 핫라인 구축 (지원 금액은 지침 원문 확인 필요)
...
1. 파견의사 심야 당직수당 지원
  - 의료취약지 파견의사 지원사업: 심야 당직수당 월 최대 500만원 추가 보조 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - 회당 지급 단가 및 휴일 가산 기준은 해당 연도 사업지침 원문에서 확인해야 합니다.

2. 신청 및 정산 절차
  - 지자체 보건 부서를 통해 사업지침에 정한 서식(당직 근무일지·진료 실적 등)을 갖추어 신청·정산합니다. 제출 기한과 기관은 해당 연도 지침을 확인하세요.`;
        } else {
          detailed_answer = `[사전 작성 정책 질의응답 요약 · AI 모델 미사용]
질문: "${query}"
...
2. 데이터 기반 지자체 현황 (${region_name})
  - 응급 60분 미도달율: ${region_stats.emergency_rate}% (${응급_판정})
  - 관내 응급환자 자체충족률: ${region_stats.ri_rate}% (${RI_판정})
...
2. 인건비 및 당직비 보조 규정 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - 전문의 파견 인건비: 1인당 국비 50%(최대 2억 5천만원 한도) + 지방비 50% 1:1 매칭
  - 심야 당직수당: 월 최대 500만원 추가 보조

3. 행정 절차
  - 협약 기간, 실적 보고 주기, 예산 신청 일정은 해당 연도 사업지침 원문을 확인해 기재하십시오.`;
...
1. 필수의료 자체충족률(RI) 산정 기준
  ...
  - 현재 수준: ${region_name} 관내 RI ${region_stats.ri_rate}% (${RI_판정})

2. 지역 공공의료기관 추진 실적보고서 초안
  [사업명] ${region_name} 필수의료 자체충족률 제고 사업
  - 목표치: 현행 ${region_stats.ri_rate}% → 차년도 목표치는 기관 여건에 맞게 설정 (참고: 취약선 미달 기관은 전년 대비 1%p 이상 개선 시 평가 만점)
  - 세부 실행계획:
    ① 야간·휴일 응급의학과 전문의 당직 체계 개편 (응급 60분 미도달율 ${region_stats.emergency_rate}%)
    ② 인근 상급종합병원과의 원격 협진 핫라인 구축
...
1. 분만취약지 선정 기준
  - 관내 분만율 40% 미만 또는 분만 가능 산부인과 60분 미도달 인구 30% 이상 시군구
  - ${분만율_판정}

2. 등급별 지원 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - A등급: 분만산부인과 개설 및 매년 5억원 운영비 연속 지원
  - B등급: 외래산부인과 설치 지원, 고위험 산모 이송체계 및 모자의료센터 핫라인 구축 (지원 금액은 원문 확인 필요)

3. ${region_name} 권고사항
  - 안전한 출산 인프라 및 고위험 산모 이송체계 연계 방안 마련`;
        } else if (query.includes('달빛어린이병원') || query.includes('야간진료') || query.includes('소아청소년과')) {
          detailed_answer = `[사전 작성 공공보건 지침 요약 · AI 모델 미사용]
■ 달빛어린이병원 지정 요건 및 야간진료 관리료 가산 규정

1. 지정 요건 (내장 지침 코퍼스 기준, 원문 확인 필요)
  - 진료 시간: 평일 야간(23시까지) 및 토·일·공휴일 최소 18시 이상 소아청소년과 전문의 진료
  - 지정 주체: 시·도지사

2. 지원 사항
  - 야간 진료관리료 가산 및 운영비 보조 (가산 금액과 보조 규모는 해당 연도 지침·고시 원문 확인 필요)
...
2. 필수의료 현안 및 데이터 진단
  - 응급 60분 미도달 인구비율: ${region_stats.emergency_rate}% (${응급_판정})
  - 관내 응급환자 자체충족률(RI): ${region_stats.ri_rate}% (${RI_판정})
...
※ 사전 작성된 요약이며 AI가 생성한 답변이 아닙니다. 수치와 기준은 앱에 내장된 지침 요약 기준이므로, 실제 신청·보고 전에 해당 연도 고시·사업지침 원문으로 반드시 확인하세요.`;
