...
- 권한이 필요한 경우 Google AI Studio(https://aistudio.google.com/app/apikey)에서 신규 키를 발급받아 재등록하시면 즉시 정상 작동합니다.`,
    ...
    const fetch_local_sllm = async () => {
      ...
      // RI 30% 기준은 플랫폼 진단 기준(공개 원문 미확인)
      const RI_판정 = (region_stats.ri_rate ?? 100) < 30 ? '플랫폼 기준 30% 미만 (취약)' : '플랫폼 기준 30% 이상';
      const 분만율_판정 =
        (region_stats.maternity_rate ?? 100) < 40
          ? `관내 분만율(플랫폼 지표)이 ${region_stats.maternity_rate}%로 낮은 편입니다. 분만취약지 해당 여부는 선정 기준인 60분 내 분만의료 이용률과 접근 불가 인구비율로 확인해야 합니다.`
          : `관내 분만율(플랫폼 지표)은 ${region_stats.maternity_rate}%입니다. 분만취약지 해당 여부는 선정 기준인 60분 내 분만의료 이용률과 접근 불가 인구비율로 확인해야 합니다.`;
...
2. 신청 자격 및 필수 요건
  ...
  - 인건비: 파견 인력 인건비의 50% 국고 지원 (1인당 한도는 연도별 사업지침 확인 — 보건복지부 안내 기준 최대 1.5억원)
  - 당직수당 별도 보조 기준은 공개 자료에서 확인되지 않음 (사업지침 원문 확인 필요)
...
2. 정부 공공보건의료 평가 가점 기준
  ...
  - 성과 평가: 공공보건의료계획 시행결과 평가는 정성평가(70점)와 정량평가(30점)로 구성되며, 자체충족률 개선 배점 기준은 공개 자료에서 확인되지 않음 (평가지침 원문 확인 필요)`;
        } else if (query.includes('분만취약지') || query.includes('A등급') || query.includes('B등급')) {
          detailed_answer = `[사전 작성 정책 질의응답 요약 · AI 모델 미사용]
질문: "${query}"

1. 분만취약지 선정 기준 및 등급별 지원
  - 선정 기준(보건복지부 분만취약지 지원사업): ① 60분 내 분만의료 이용률 30% 미만, ② 60분 내 분만기관 접근 불가 인구 30% 이상 → 둘 다 해당 A등급, 하나 해당 B등급
  - 지원: 분만산부인과 설치 시 시설·장비비 10억원(첫해) + 운영비 연 5억원, A·B등급 지역의 기존 분만 산부인과도 운영비 지원 대상 (보건복지부 제도 개선 보도자료 기준)
...
1. 파견의사 당직수당 지원
  - 공공병원 파견 의료인력 인건비 지원사업은 인건비의 50%를 국고 지원하며, 당직수당 별도 보조 기준은 공개 자료에서 확인되지 않았습니다.
  - 당직비 지급 단가와 휴일 가산 기준은 해당 연도 사업지침·기관 규정 원문에서 확인해야 합니다.
...
2. 인건비 보조 규정
  - 파견 인력 인건비의 50% 국고 지원 (1인당 한도는 연도별 사업지침 확인 — 보건복지부 안내 기준 최대 1.5억원)
  - 당직수당 별도 보조 기준은 공개 자료에서 확인되지 않음 (사업지침 원문 확인 필요)
...
2. 지역 공공의료기관 추진 실적보고서 초안
  ...
  - 목표치: 현행 ${region_stats.ri_rate}% → 차년도 목표치는 기관 여건과 해당 연도 평가지침에 맞게 설정
  - 세부 실행계획:
...
1. 운영 기준 (보건복지부 달빛어린이병원 운영지침 기준)
  - 최소 운영시간: 평일 18시~23시, 토·일·공휴일 10시~18시

2. 지원 사항
  - 소아 야간진료관리료: 운영시간에 따라 건당 1만 3,390원~2만 2,600원 가산 (협력약국 야간조제관리료 3,980원 별도)
  - 운영비: 운영시간 비례 연 3,000만원~4억 3,200만원 (소아 인구 3만 명 미만 지역 20% 가산 포함)
