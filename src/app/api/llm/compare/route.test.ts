...
describe('사전 작성 템플릿 답변 (로컬 sLLM 미연결)', () => {
  ...
  it.each(질의_목록)('"%s"(%s): 코퍼스와 불일치·미확인 수치와 영월 전용 표현을 쓰지 않는다', async (q, mode) => {
    const r = await 질의(q, mode, 취약);
    for (const v of ['2,160', '2,910', '20만~30만', '40만~50만', '10억', '1억~1.5억', '11월', '익월 10일', '원주세브란스', '영월의료원', '영월 권역']) {
  ...
  it('취약 지역에는 실제 값에 따른 취약 판정을 표시한다', async () => {
    ...
    expect(r.response).toContain('19.8% (기준선 30% 미만 (취약))');
  });

  it('분만율이 기준 이상이면 분만율 기준 미해당으로 안내한다', async () => {
    const r = await 질의('분만취약지 A등급', 'general_qa', 양호);
    expect(r.response).toContain('분만율 기준(40% 미만)에는 해당하지 않으며');
  });
});
